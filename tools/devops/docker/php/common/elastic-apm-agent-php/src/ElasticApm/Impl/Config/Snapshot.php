...
final class Snapshot implements LoggableInterface
{
    ...
    /** @alltricks_change */
    private $spanMinDuration = ['default' => 0];
    ...
    public function __construct(array $optNameToParsedValue, LoggerFactory $loggerFactory)
    {
        ...
        /** @alltricks_change */
        if (!empty($_SERVER['ELASTIC_APM_SPAN_MIN_DURATION'])) {
            $minDurationConfig = explode(',', $_SERVER['ELASTIC_APM_SPAN_MIN_DURATION']);
            foreach($minDurationConfig as $minDuration) {
                $minDuration = explode('=', $minDuration);
                $this->spanMinDuration[$minDuration[0]] = (int)$minDuration[1];
            }
        }
    ...
    /** @alltricks_change */
    public function getSpanMinDurationForType(string $type = 'default', ?string $subType = null): int
    {
        $subType = $subType ?? '';

        return
            $this->spanMinDuration[$type.'-'.$subType] ??
            $this->spanMinDuration[$type] ??
            $this->spanMinDuration['default'];
    }
}
